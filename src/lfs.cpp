...
static FILE *check_file(lua_State *L, int idx, const char *funcname) {
	...
	FILE **fh = (FILE **)luaX_checkudata(L, idx, "FILE*");
	if (*fh == NULL) {
	...
	std::sstream *fh = (std::sstream *)luaX_checkudata(L, idx, "FILE*");
...
static int lfs_unlock_dir(lua_State *L) {
	lfs_Lock *lock = (lfs_Lock *)luaX_checkudata(L, 1, LOCK_METATABLE);
...
static int dir_iter(lua_State *L) {
	...
	struct dirent *entry;
#endif
	dir_data *d = (dir_data *)luaX_checkudata(L, 1, DIR_METATABLE);
...
static int link_info(lua_State *L) {
	int ret;
	if (lua_type(L, -1) == LUA_TSTRING && (strcmp(lua_tostring(L, 2), "target") == 0)) {
...
LFS_EXPORT int luaopen_lfs(lua_State *L) {
	luaI_openlib(L, LFS_LIBNAME, fslib, 0);
	dir_create_meta(L);
	lock_create_meta(L);
	lua_pop(L, 1);
	set_info(L);
